    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="sticky top-0 z-20 bg-slate-950/95 backdrop-blur border-b border-slate-800 px-4">
        <div className="max-w-lg mx-auto flex items-center justify-between py-3">
          ...
          <Link href="/saved" className="text-xs text-slate-400 bg-slate-800 rounded-full px-3 py-1.5 hover:bg-slate-700 transition-colors">
            Saved
          </Link>
        </div>
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-xs font-semibold transition-all ${
                activeTab === tab.value ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800"
              }`}
            >
              {tab.icon} {tab.label}
            </button>
      ...
      <main className="max-w-lg mx-auto px-4 py-4 flex flex-col gap-3">
        {activeTab === "INSTANT_WIN" && topAlerts.length > 0 && (
          <div className="rounded-2xl bg-amber-500/5 border border-amber-500/20 p-3">
            <p className="text-[10px] font-bold text-amber-400 uppercase tracking-widest mb-2">High-Score Alerts</p>
            {topAlerts.map((o) => (
              <Link key={o.id} href={`/opportunity/${o.id}`} className="flex items-center justify-between py-1.5 hover:opacity-80 transition-opacity">
        ...
        {tabOpps.length === 0 ? (
          <div className="text-center py-16 text-slate-500 text-sm">No opportunities here — check another tab.</div>
        ) : (
          tabOpps.map((opp) => (
