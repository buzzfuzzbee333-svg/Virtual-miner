...
export default function SavedPage() {
  ...
  function remove(id: string) {
    setInteraction(id, "DISMISSED");
    refresh();
  }

  function markComplete(id: string) {
    setInteraction(id, "COMPLETED");
    refresh();
  }
              ...
              {saved.map((opp) => (
                <div key={opp.id} className="rounded-2xl bg-slate-900 border border-slate-800 p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/opportunity/${opp.id}`} className="text-xs font-semibold text-white line-clamp-1 hover:text-indigo-400 transition-colors">
                      {opp.title}
                    </Link>
                  ...
                  <div className="flex flex-col gap-1">
                    <button
                      onClick={() => markComplete(opp.id)}
                      className="text-[10px] px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-semibold hover:bg-emerald-500/20 transition-colors"
                    >
                      Done
                    </button>
                    <button
                      onClick={() => remove(opp.id)}
                      className="text-[10px] px-2 py-1 rounded-lg bg-slate-800 text-slate-500 hover:text-slate-300 transition-colors"
                    >
                      Remove
                    </button>
                  </div>
