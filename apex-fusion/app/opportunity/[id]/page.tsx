      ...
      <main className="max-w-lg mx-auto px-4 py-5 flex flex-col gap-5">
              ...
              <div className="flex gap-4 mt-2">
                {opp.estimatedValue && (
                  <div>
                    ...
                    <p className="text-sm font-bold text-emerald-400">{opp.estimatedValue}</p>
                  </div>
                )}
                {opp.estimatedTime && (
                  <div>
                    <p className="text-[10px] text-slate-500">Time</p>
                    <p className="text-sm font-bold text-sky-400">{opp.estimatedTime}</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="rounded-2xl bg-slate-900 border border-slate-800 p-4">
          ...
          <p className="text-[10px] text-slate-600 mt-3 leading-relaxed">
            Scores are weighted: Payout (30%) · Ease (25%) · Speed (15%) · Demand (15%) · Competition (10%) · Risk (-5%)
          </p>
        </div>
            ...
            <div className="flex flex-col gap-3">
              {opp.steps.map((step, i) => (
                <div key={i} className="flex gap-3">
                  <span className="w-5 h-5 rounded-full bg-indigo-600/20 border border-indigo-600/40 flex items-center justify-center text-[10px] font-bold text-indigo-400 shrink-0 mt-0.5">
                    {i + 1}
                  </span>
        ...
        <div className="flex flex-wrap gap-1.5">
          {opp.tags.map((tag) => (
            <span key={tag} className="text-[10px] px-2 py-1 rounded-full bg-slate-800 text-slate-500 font-medium">#{tag}</span>
          ))}
        </div>
        <div className="flex flex-col gap-2 pb-8">
          {status === "COMPLETED" ? (
            <div className="w-full py-3 rounded-2xl bg-emerald-500/10 text-emerald-400 text-sm font-semibold text-center border border-emerald-500/20">
              Marked as Complete ✓
            </div>
          ) : (
            <button
              onClick={() => interact("COMPLETED")}
              className="w-full py-3 rounded-2xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-500 transition-colors"
            >
              Mark as Complete
            </button>
          )}
          {status !== "SAVED" && status !== "COMPLETED" ? (
            <button
              ...
              className="w-full py-3 rounded-2xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-500 transition-colors"
            >
              Save Opportunity
            </button>
          ) : null}
          <button
            onClick={() => { interact("DISMISSED"); router.back(); }}
            className="w-full py-3 rounded-2xl bg-slate-800 text-slate-400 text-sm font-semibold hover:bg-slate-700 transition-colors"
          >
            Dismiss
          </button>
        </div>
