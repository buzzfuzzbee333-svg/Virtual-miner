...
class Scorer:
    # --- Thresholds (tune these after real data comes in) ---
    MIN_SUCCESS_RATE  = 0.30   # below 30% → disable
    MIN_DAILY_USD     = 0.001  # below $0.001/day → disable
    MIN_RUNS_TO_SCORE = 5      # don't score until N runs collected
    ...
    def score_task(self, task_name: str) -> dict:
        ...
        # Composite score:
        #   50 pts from success rate  (100% → 50 pts)
        #   50 pts from daily value   (≥$0.01/day → full 50 pts)
    ...
    def audit_all(self) -> list[dict]:
        """Score every task that has run history."""
        with self._conn() as c:
            names = c.execute(
    ...
    def auto_disable(self, registry: "TaskRegistry") -> list[str]:
        """
        Disable tasks whose recommendation starts with 'disable'.
        Returns the list of task names that were disabled.
        """
        disabled = []
